...
import { User as UserIcon, Camera, Save, Loader2, Shield } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateUserProfile(displayName, photoURL, adminCode);
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label htmlFor="adminCode" className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                <Shield className="w-4 h-4 mr-1 text-gray-400" />
                Codi d'Administrador
              </label>
              {isAdmin ? (
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-shadow"
                  placeholder="Introdueix el codi si ets l'administrador"
                />
              )}
            </div>
